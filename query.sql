...
AS SELECT cds, ad, appelli.adcod, sum(Iscrizione) AS n_iscritti, SUM(Superamento) AS n_promossi
FROM appelli
...
ALTER TABLE tasso_superamento
     ADD tasso_superamento AS(CAST (n_promossi AS float) / n_iscritti)
...
--- Denormalizzata:
CREATE TABLE sup_denorm AS  
SELECT CdS, AD, AdCod, sum(Iscrizione) as n_iscritti, sum(Superamento) as n_promossi 
